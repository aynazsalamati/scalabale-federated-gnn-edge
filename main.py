...
def evaluate_global_model(model, data, device="cpu"):
    ...
    with torch.no_grad():
        ...
        if hasattr(data, "test_mask") and data.test_mask is not None and int(data.test_mask.sum()) > 0:
...
def run_single_method(
        ...
        for client in clients:
            ...
            if should_send:
                ...
                if config["topology_aware"]:
                    ...
                    importance = topology.get_client_weight(
                        client.client_id
                    )
    ...
    history = {
        ...
        "cumulative_communication": list(
            np.cumsum(communication)
        ),
...
def average_histories(histories):
    ...
    for key in metric_keys:
        ...
        averaged[key] = values.mean(
            axis=0
        ).tolist()

        averaged[f"{key}_std"] = values.std(
            axis=0
        ).tolist()
...
def make_cache_name(
    ...
    return (
        ...
        f"alpha_{alpha}_"
        f"reddit_sample_{reddit_sample_nodes}.pkl"
    )
    ...
    for dataset_name in DATASETS:
        ...
        if RUN_GRID:
            ...
            for n_clients in GRID_CLIENT_SCENARIOS:
                ...
                panel_results[
                    f"{dataset_name} - {n_clients} Clients"
                ] = scenario_results

            plot_metric_grid(
                panel_results=panel_results,
                metric_key="accuracy",
                title=f"Global Accuracy Across Client Scenarios",
                ylabel="Accuracy",
            ...
            plot_metric_grid(
                panel_results=panel_results,
                metric_key="loss",
                title=f"Training Loss Across Client Scenarios",
                ylabel="Loss",
            ...
            plot_metric_grid(
                ...
                title=f"Cumulative Communication Cost Across Client Scenarios",
